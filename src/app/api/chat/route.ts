      ...
      tools: {
        ...
        save_diary_entry: tool({
          ...
          parameters: z.object({
            ...
            date: z.string().describe('YYYY-MM-DD 格式的日期（用"今天"对应实际日期）'),
        ...
        update_diary_entry: tool({
          ...
          parameters: z.object({
            ...
            date: z.string().optional().describe('YYYY-MM-DD 格式的日期'),
            summary: z.string().optional().describe('一句话总结'),
          }),
          execute: async (args) => {
            if (!userId) return "用户未登录";
