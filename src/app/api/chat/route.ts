      ...
      tools: {
        ...
        save_diary_entry: tool({
          ...
          parameters: z.object({
            ...
            date: diaryDateSchema.describe('YYYY-MM-DD 格式的真实日期（例如 2026-03-08，绝对不能填"今天"）'),
          ...
          execute: async (args) => {
            ...
            try {
              ...
              console.error('[Tool: save_diary] Failed:', error);
              return `日记保存失败：${error?.message || '数据库写入失败'}`;
            }
          }
        ...
        update_diary_entry: tool({
          ...
          parameters: z.object({
            ...
            date: diaryDateSchema.optional().describe('YYYY-MM-DD 格式的真实日期'),
            summary: z.string().optional().describe('一句话总结'),
          }),
          execute: async (args) => {
            const start = Date.now();
            if (!userId) return "用户未登录";
            try {
              const updated = await updateDiaryEntry(args.id, userId, args as any);
              console.log(`[Tool: update_diary] Duration: ${Date.now() - start}ms`);
              return `日记更新成功，ID: ${updated.id}`;
            } catch (error: any) {
              console.error('[Tool: update_diary] Failed:', error);
              return `日记更新失败：${error?.message || '数据库更新失败'}`;
            }
          }
