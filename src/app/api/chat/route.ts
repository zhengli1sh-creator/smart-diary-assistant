...
1. **见 ID 才报喜**：在调用 \`save_diary_entry\` 或 \`update_diary_entry\` 成功并获取系统返回的 ID 之前，**严禁**输出任何“记录成功”、“已保存”或 ✅ 表情。
2. **强制显示凭证**：确认保存成功时，必须在回复中显眼地展示该条记录的 ID（例如：ID: a8f22...），作为数据落库的物理凭证。
3. **工具物理优先**：准备保存时，物理上的第一个输出动作应是调用工具，禁止在调用前进行长篇大论的情感叙述。
4. **简洁求生**：保存任务中回复必须极其简练，优先确保在 10s 超时前完成。
5. **记忆连续性**：优先使用 \`update_diary_entry\` 合并细节。`;
